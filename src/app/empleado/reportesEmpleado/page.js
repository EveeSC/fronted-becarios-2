...
export default function ReportesEmpleado() {
  ...
  const descargarReportePagos = async () => {
    try {
      const token = localStorage.getItem("token")
      const response = await axios.get("http://localhost:3031/api/reporte_pagos_excel", {
        headers: {
          Authorization: `Bearer ${token}`,
        },
        responseType: "blob",
      ...
      const link = document.createElement("a")
      link.href = url
      link.setAttribute("download", "reporte_pagos.xlsx")
      document.body.appendChild(link)
      link.click()
      link.remove()
    } catch (error) {
  ...
  //Descargar reporte de pagos por mes
  const descargarReportePagosMes = async () => {
    try {
      const token = localStorage.getItem("token")
      const response = await axios.get("http://localhost:3031/api/reporte_pagos_mes", {
        headers: {
          Authorization: `Bearer ${token}`,
        },
        responseType: "blob",
      ...
      const link = document.createElement("a")
      link.href = url
      link.setAttribute("download", "reporte_pagos_mes.xlsx")
      document.body.appendChild(link)
      link.click()
      link.remove()
    } catch (error) {
        ...
        <TabsContent value="pagos">
          <Card>
            <CardHeader className="flex flex-row items-center justify-between">
              ...
              <Button variant="outline" onClick={descargarReportePagos} className="flex items-center gap-2">
...
<TabsContent value="pagosMes">
  <Card>
    <CardHeader className="flex flex-row items-center justify-between">
      <CardTitle>Pagos del Mes Actual</CardTitle>
      <Button variant="outline" onClick={descargarReportePagosMes} className="flex items-center gap-2" >
        <Calendar size={16} />
        Descargar Reporte Mensual
      </Button>
    </CardHeader>
